...
// Stolen from src/system/unix-glx/resource.c
...
int main(int argc, char *argv[]) {
    ...
    if(argc > 4) {
        ...
        header.type = BBRES_UNKNOWN;
        if(input = fopen(argv[3], "rb")) {
            if(output = fopen(argv[4], "wb")) {
                fseek(input, 0, SEEK_END);
                header.size = ftell(input);
                fseek(input, 0, SEEK_SET);
                header.hash = 0xFFFFFFFF;
                fwrite(&header, sizeof(breadbox_resource_header_t), 1, output);
                while((temp = fgetc(input)) != EOF) {
                    header.hash = CRC32C_TABLE[(char)header.hash ^ (char)temp] ^ (header.hash >> 8);
                    fputc(temp, output);
                }
                header.hash ^= 0xFFFFFFFF;
